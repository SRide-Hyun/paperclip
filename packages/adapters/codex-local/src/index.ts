...
const CODEX_LOCAL_GPT_6_REASONING_EFFORTS = [
  "low",
  "medium",
  "high",
  "xhigh",
...
export type CodexLocalReasoningEffort =
  ...
  | (typeof CODEX_LOCAL_GPT_6_REASONING_EFFORTS)[number];
...
export function codexLocalReasoningEffortsForModel(
  ...
  return normalizedModel === "gpt-6-astra" || normalizedModel === "gpt-6-sol"
    ? CODEX_LOCAL_GPT_6_REASONING_EFFORTS
...
- modelReasoningEffort (string, optional): reasoning effort override passed via -c model_reasoning_effort=...; GPT-6 Astra and Sol support low|medium|high|xhigh|max|ultra
- promptTemplate (string, optional): run prompt template
- search (boolean, optional): run codex with --search
- fastMode (boolean, optional): enable Codex Fast mode; supported on GPT-6 Astra and Sol, GPT-5.6 (sol/terra/luna), GPT-5.5, GPT-5.4 and passed through for manual model IDs
...
- Fast mode is supported on GPT-6 Astra and Sol, GPT-5.6 (sol/terra/luna), GPT-5.5, GPT-5.4 and manual model IDs. When enabled for those models, Paperclip applies \`service_tier="fast"\` and \`features.fast_mode=true\`.
